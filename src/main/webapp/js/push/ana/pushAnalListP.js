...
/*// 목록에서 메일명 클릭시 팝업창
function goPushSendList(pushmessageId, rsltCd) { 
	$("#popPushSendResultSearchForm input[name='sendTelNo']").val("");
	$("#popPushSendResultSearchForm input[name='rsltCd']").val(rsltCd);
	$("#popPushSendResultSearchForm input[name='pushmessageId']").val(pushmessageId);
	
	goPushSendReultList();
	fn.popupOpen('#popup_pushSendResultList');
}
*/

function goPushPreview() {
	// 미리보기 관련 설정 초기화
	var previewHtml = "";
	...
	fn.popupOpen('#popup_push_preview');
}
