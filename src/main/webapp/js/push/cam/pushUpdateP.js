...
// 수신자그룹을 선택하였을 경우 머지부분 처리 (수신자 선택이 미리 되어 있는 경우 선제적으로 그리게 되어 있다.)
function goMerge() {
	if($("#segNoc").val() == "") {
		alert("수신자그룹을 선택하세요.");
		return;
...
function goTitleMerge() {
	...
	if($("#pushName").val() == "") {
		$("#pushName").val( $("#mergeKeyPush").val() );
	} else {
		var cursorPosition = $("#pushName")[0].selectionStart;
		$("#pushName").val( $("#pushName").val().substring(0,cursorPosition) + $("#mergeKeyPush").val() + $("#pushName").val().substring(cursorPosition) );
	}
	setPhonePreview();
}
	...
	if( $("#txtCallUri").val().length > 1 || $("#txtCallUriIos").val().length > 1 ){
		if ($("#txtCallUri").val().length > 1 && $("#txtCallUriIos").val().length > 1){
			if($("#txtCallUri").val().trim() != $("#callUri").val().trim() ) {
				alert("Android항목의 URL유효성을 확인해주세요");
				return true; 
			}
			if($("#txtCallUriIos").val().trim() != $("#callUriIos").val().trim() ) {
				alert("iOS항목의 URL유효성을 확인해주세요");
				return true; 
			}
		} else{
...
// 등록 클릭시 (PUSH 수정) => 수정 필요
function goPushUpdate() {
	...
	$("#pushInfoForm").attr("target","iFrmMail").attr("action","./pushUpdate.ums").submit();
}

// 발송승인 클릭시 (발송 승인 상태 변경)
function goPushAdmit() {
...
function popUrlPreview() {
	...
	if( $("#txtCallUri").val().trim().length < 1) {
		alert("Android URL을 입력해주세요");
	} else {
		url = $("#txtCallUri").val().trim();
	}
	
	if( $("#txtCallUriIos").val().trim().length < 1) {
		alert("IOS URL을 입력해주세요");
	} else {
		urlIos = $("#txtCallUriIos").val().trim();
	}
	
	if(!validUrl(url) ){
		alert("Android URL이 올바르지 않습니다");
		return; 
	} else {
		$("#callUri").val(url);
	}
	...
	if(!validUrl(urlIos) ){
		alert("IOS URL이 올바르지 않습니다");
		return; 
	}else {
		$("#callUriIos").val(urlIos);
	}
	...
	$("#iFrmUrlAnd").empty();
	$("#previewUrlAnd").html(url);
	$("#previewUrlTypeAnd").html(urlTypeNm);
	
	$("#iFrmUrlIos").empty();
	$("#previewUrlIos").html(urlIos);
	$("#previewUrlTypeIos").html(urlTypeNm);
	
	var param = $("#pushInfoForm").serialize();
	$.getJSON("./urlPreview.json?" + param, function(res) {
		if(res.result == 'Success') {
			document.getElementById("iFrmUrlAnd").src= res.uri;
			document.getElementById("iFrmUrlIos").src= res.uriIos;
