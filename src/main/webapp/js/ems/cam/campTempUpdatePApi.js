...
function getUserInfo(userId){
	if(userId != "") {
		$.getJSON("/sys/acc/userInfo.json?userId=" + userId, function(data) { 
			if(data.result == "Success"){
				var mailFromNm = 	data.userInfo.mailFromNm;
				var mailFromEm = 	data.userInfo.mailFromEm;
				$("#campTempSid").val(userId);
				$("#campTempSname").val(mailFromNm);
				$("#campTempSmail").val(mailFromEm);
			} 
	}); 
	}
}
...
function goUpdate() {
	...
	*/
	...
	$("#campTempInfoForm").attr("target","iFrmMail").attr("action","/ems/cam/campTempUpdateApi.ums").submit();
}
