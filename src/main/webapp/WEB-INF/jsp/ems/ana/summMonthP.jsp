			...
			<tbody>
				...
				<c:if test="${fn:length(monthList) > 0}">
					...
					<c:forEach items="${monthList}" var="month">
						...
						<tr>
							<td><c:out value="${fn:substring(month.ymd,0,4)}-${fn:substring(month.ymd, 4,6)}"/></td>
							<td>
								<fmt:formatNumber var="sendCntNum" type="number" value="${month.sendCnt}" /><c:out value="${sendCntNum}"/>
							</td>
							<td>
								<fmt:formatNumber var="succCntNum" type="number" value="${month.succCnt}" /><c:out value="${succCntNum}"/>
							</td>
							<td>
								<fmt:formatNumber var="failCntNum" type="number" value="${month.sendCnt - month.succCnt}" /><c:out value="${failCntNum}"/>
							</td>
							<td>
								<fmt:formatNumber var="openCntNum" type="number" value="${month.openCnt}" /><c:out value="${openCntNum}"/>
							</td>
							<td>
								<fmt:formatNumber var="validCntNum" type="number" value="${month.validCnt}" /><c:out value="${validCntNum}"/>
							</td>
							<td>
								<fmt:formatNumber var="clickCntNum" type="number" value="${month.clickCnt}" /><c:out value="${clickCntNum}"/>
							</td>
							<td>
								<fmt:formatNumber var="blockCntNum" type="number" value="${month.blockCnt}" /><c:out value="${blockCntNum}"/>
							</td>
						</tr>
					</c:forEach>
					<!-- //데이터가 있을 경우 -->
