...
<div class="graybox">
	<div class="title-area">
		...
		<!-- 버튼// -->
		<div class="btn-wrap">
			<button type="button" class="btn fullorange plus" onclick="goAdd();">신규등록</button>
		</div>
		<!-- //버튼 -->
	</div>
				...
				<c:if test="${fn:length(campaignList) > 0}">
					<c:forEach items="${campaignList}" var="campaign" varStatus="campStatus">
						<tr>
							...
							<td><a href="javascript:goPushList('<c:out value='${campaign.campNo}'/>');" class="regular"><c:out value="${campaign.campPushCnt}"/>건</a></td>
