...
<script type="text/javascript" src="<c:url value='/js/sms/ana/campKakaoListP.js'/>"></script>
<body>
												...
												<select id="searchTempCd" name="searchTempCd" title="템플릿 선택">
													...
													<c:if test="${fn:length(tempList) > 0}">
														<c:forEach items="${tempList}" var="temp">
															<option value="<c:out value='${temp.cd}'/>"<c:if test="${temp.cd eq searchVO.searchTempCd}"> selected</c:if>><c:out value='${temp.cdNm}'/></option>
														</c:forEach>
													</c:if>
												</select>
											</div>
