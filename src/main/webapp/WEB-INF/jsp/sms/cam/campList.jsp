...
<div class="graybox">
	<div class="title-area">
		...
		<!-- 버튼// -->
		<div class="btn-wrap">
			<button type="button" class="btn fullgreen plus" onclick="goAdd();">신규등록</button>
		</div>
		<!-- //버튼 -->
	</div>
				...
				<c:if test="${fn:length(campaignList) > 0}">
					<c:forEach items="${campaignList}" var="campaign" varStatus="campStatus">
						<tr>
							...
							<td><c:out value="${campaign.campNo}"/></td>
							<td>
								<a href="javascript:goUpdate('<c:out value='${campaign.campNo}'/>');" class="bold"><c:out value='${campaign.campNm}'/></a>
							</td>
							<td>
								<c:if test="${campaign.campSmsCnt > 0 }">
									<a href="javascript:goSmsList('<c:out value='${campaign.campNo}'/>');" class="regular"><c:out value="${campaign.campSmsCnt}"/>건</a>
								</c:if>
								<c:if test="${campaign.campSmsCnt == 0 }">
									0건
								</c:if>
