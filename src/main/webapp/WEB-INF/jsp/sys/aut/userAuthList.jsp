				...
				<c:if test="${fn:length(userAuthList) > 0}">
					<c:forEach items="${userAuthList}" var="userAuth" varStatus="userAuthStatus">
						<tr>
							...
							<td align="center">
								<label for="checkbox_<c:out value='${userAuthStatus.count}'/>"><input type="checkbox" id="checkbox_<c:out value='${userAuthStatus.count}'/>" name="delUserId" value="<c:out value='${userAuth.userId}'/>"><span></span></label>
							</td>
							<td>
								<a href="javascript:goUpdate('<c:out value='${userAuth.userId}'/>')" class="bold"><c:out value='${userAuth.userId}'/></a>
							</td>
							<td>
								<a href="javascript:goUpdate('<c:out value='${userAuth.userId}'/>')" class="bold"><c:out value='${userAuth.userNm}'/></a>
							</td>
							<td><c:out value="${userAuth.statusNm}"/></td>
							<td><c:out value="${userAuth.deptNm}"/></td>
							<td> 
								<a href="javascript:popUserAuthSearch('<c:out value='${userAuth.userId}'/>')" class="bold"><c:out value='${userAuth.menuCount}'/></a>
							</td>
