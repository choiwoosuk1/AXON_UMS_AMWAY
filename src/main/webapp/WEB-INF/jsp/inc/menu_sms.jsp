...
<span class="logo-ums"><a href="/service.ums"><img src="/img/sms/logo_ums.png" alt="UMS"></a></span>
<h1>AXon SMS</h1>

<div class="btn-mail">
	<button type="button" class="btn fullgreen" onclick="javascript:goNewSms();">신규발송</button>
</div>
	...
	<ul>
		<c:if test="${fn:length(NEO_MENU_LIST) > 0}">
			<c:forEach items="${NEO_MENU_LIST}" var="lvl1">
				<c:if test="${lvl1.menulvlVal == 1 && lvl1.serviceGb == 30}">
					<li<c:if test="${lvl1.menuId eq NEO_P_MENU_ID}"> class="active"</c:if>>
						<c:set var="pMenuClass" value=""/>
						<c:choose>
							<c:when test="${'M3001000' eq lvl1.menuId}"><c:set var="pMenuClass" value="item01"/></c:when>
							<c:when test="${'M3002000' eq lvl1.menuId}"><c:set var="pMenuClass" value="item02"/></c:when>
							<c:when test="${'M3003000' eq lvl1.menuId}"><c:set var="pMenuClass" value="item03"/></c:when>
							<c:when test="${'M3004000' eq lvl1.menuId}"><c:set var="pMenuClass" value="item04"/></c:when>
						</c:choose>
