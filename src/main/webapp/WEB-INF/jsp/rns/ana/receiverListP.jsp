...
<script type="text/javascript" src="<c:url value='/js/rns/ana/receiverListP.js'/>"></script>
			...
			<section class="cont-body">
				...
				<form id="searchForm" name="searchForm" method="post">
					...
					<fieldset>
							...
							<div class="list-area">
								<ul>
									...
									<li>
										...
										<div class="list-item">
											<div class="select">
												<select id="searchRcode" name="searchRcode" title="옵션 선택">
													<option value="">선택</option>
													<c:if test="${fn:length(rcodeList) > 0}">
														<c:forEach items="${rcodeList}" var="rcode">
															<option value="<c:out value='${rcode.cd}'/>"<c:if test="${rcode.cd eq searchVO.searchRcode}"> selected</c:if>><c:out value='${rcode.cdNm}'/></option>
														</c:forEach>
													</c:if>
												</select>
											</div>
									...
									<li>
										<label>수신자ID</label>
										<div class="list-item">
											<input type="text" id="searchRid" name="searchRid" placeholder="수신자ID를 입력해주세요.">
										</div>
									</li>
									<li>
										<label>수신자성명</label>
										<div class="list-item">
											<input type="text" id="searchRname" name="searchRname" placeholder="수신자성명을 입력해주세요.">
										</div>
									</li>
									<li>
										<label>이메일주소</label>
										<div class="list-item">
											<input type="text" id="searchRmail" name="searchRmail" placeholder="이메일주소를 입력해주세요.">
										</div>
									</li>
									<li>
										<label>서비스</label>
						...
						<!-- 목록&페이징// -->
						<div id="divReceiverList" style="margin-top:50px;"></div>
						<!-- //목록&페이징 -->
